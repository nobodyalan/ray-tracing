...
Light *SceneParser::parseDirectionalLight() {
    ...
    // 💡 核心对齐：将基础颜色乘以强度系数，作为最终的辐射照度注入物理内核
...
Light *SceneParser::parsePointLight() {
    ...
    // 💡 核心对齐：基础颜色乘以强度系数，作为最终的中心辐射强度注入物理内核
...
Light *SceneParser::parseSpotLight() {
    ...
    getToken(token);
    assert (!strcmp(token, "innerAngle"));
    float innerAngle = readFloat();
    
    getToken(token);
    assert (!strcmp(token, "outerAngle"));
    float outerAngle = readFloat();
    
    getToken(token);
    assert (!strcmp(token, "}"));
    
    // 💡 核心对齐：基础颜色乘以强度系数传入聚光灯构造函数
    return new SpotLight(position, direction, color * intensity, innerAngle, outerAngle);
}
...
Light *SceneParser::parseAreaLight() {
    ...
    Vector3f position = readVector3f();
    getToken(token);
    assert (!strcmp(token, "dir1"));
    Vector3f dir1 = readVector3f();
    getToken(token);
    assert (!strcmp(token, "dir2"));
    Vector3f dir2 = readVector3f();
    getToken(token);
    assert (!strcmp(token, "color"));
    Vector3f color = readVector3f();
    getToken(token);
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    return new AreaLight(position, dir1, dir2, color*intensity);
}
// ====================================================================
// ====================================================================
...
Material *SceneParser::parseMaterial() {
    ...
    // 💡 3. 将解析出来的 transparency 和 roughness 传入构造函数
    auto *answer = new Material(diffuseColor, specularColor, shininess, reflectiveColor, refractiveIndex, emissionColor, transparency, roughness);
    return answer;
}
...
Cube *SceneParser::parseCube() {
    ...
    while (true) {
        getToken(token);
        if (!strcmp(token, "min_p")) {
            min_p = readVector3f();
        } else if (!strcmp(token, "max_p")) {
            max_p = readVector3f();
        } else {
            assert(!strcmp(token, "}"));
            break;
    ...
    // 实例化对应的 Cube 几何对象，并附加当前激活的材质指针
    return new Cube(min_p, max_p, current_material);
}
